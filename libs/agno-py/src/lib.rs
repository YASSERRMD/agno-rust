...
fn init_tracing_py(service_name: String, otlp_endpoint: Option<String>) -> PyResult<()> {
    agno_rust::init_tracing(&service_name, otlp_endpoint.as_deref())
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))
}

#[pyfunction]
fn current_span_attributes_py(labels: PyTelemetryLabels) {
    agno_rust::current_span_attributes(&labels.inner);
}

#[pyfunction]
fn flush_tracer_py() {
    agno_rust::flush_tracer();
}

#[pyfunction]
fn span_with_labels_py(_name: String, labels: PyTelemetryLabels) {
    let _span = agno_rust::span_with_labels("python", &labels.inner);
}
...
stub_pyclass!(PyAgnoError, "AgnoError");
...
#[pymodule]
fn agno(py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Agent>()?;
